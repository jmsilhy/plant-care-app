...
export function useSupabasePlants(user, supabase) {
  ...
  const addPlant = async (plantData) => {
    ...
    try {
      ...
      const dbPlantData = {
        ...
        acquisition_date: plantData.acquisition_date || plantData.acquisitionDate, // Handle both cases
  ...
  const updatePlant = async (plantId, updates) => {
    ...
    try {
      ...
      if (updates.acquisition_date !== undefined) dbUpdates.acquisition_date = updates.acquisition_date;
      if (updates.acquisitionDate !== undefined) dbUpdates.acquisition_date = updates.acquisitionDate; // Handle camelCase input
      if (updates.source !== undefined) dbUpdates.source = updates.source;
      if (updates.price !== undefined) dbUpdates.price = updates.price;
      if (updates.notes !== undefined) dbUpdates.notes = updates.notes;
      if (updates.cover_photo_id !== undefined) dbUpdates.cover_photo_id = updates.cover_photo_id;
      if (updates.coverPhotoId !== undefined) dbUpdates.cover_photo_id = updates.coverPhotoId; // Handle camelCase input
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          plant_id: plantId,
          user_id: user.id,
          file_name: photoData.fileName || photoData.file_name, // Handle both cases
          storage_path: photoData.fileName || photoData.file_name || photoData.storage_path, 
        }])
        .select()
        .single();
  ...
  const setCoverPhoto = async (plantId, photoId) => {
    console.log('🖼️ setCoverPhoto called:', { plantId, photoId });
    ...
    try {
      ...
      console.log('🖼️ Cover photo update result:', { data, error });
