...
function ExpandablePlantCard({ 
  ...
  // Auto-collapse on scroll with buffer
  useEffect(() => {
    if (!isExpanded) return;

    let initialScrollY = window.scrollY;
    const SCROLL_THRESHOLD = 200; // pixels user needs to scroll before closing

    const handleScroll = () => {
      const currentScrollY = window.scrollY;
      const scrollDistance = Math.abs(currentScrollY - initialScrollY);
      
      // Only close if user has scrolled more than threshold
      if (scrollDistance > SCROLL_THRESHOLD) {
        setIsExpanded(false);
      }
    };

    // Capture initial scroll position when card expands
  ...
  return (
    <div 
      ...
      } ${isExpanded ? 'shadow-xl scale-[1.02] z-10 relative' : ''} ${
      ...
      <div className="p-4">
        ...
        {!isExpanded && (
          <p className={`text-xs truncate ${
            bulkCareMode && !isSelected ? 'text-gray-400' : 'text-gray-500'
          }`}>
          ...
          <div className="space-y-2 mb-4">
            ...
            {plant.notes && (
              <p className={`text-xs italic ${
                bulkCareMode && !isSelected ? 'text-gray-300' : 'text-gray-500'
              }`}>
                {plant.notes}
              </p>
            )}
