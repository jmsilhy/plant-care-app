...
import ResponsivePlantCard from './components/ResponsivePlantCard';
...
function PlantManager() {
  const { user, profile, supabase, signIn, signUp, resetPassword,signOut } = useAuth();
  const { plants, addPlant, updatePlant, deletePlant, addPhotoToPlant, setCoverPhoto, updatePhotoOrder, deletePhotoFromPlant, fetchPlants } = useSupabasePlants(user, supabase);
  ...
  const handleDeleteCareActivity = async (activity_id) => {
  try {
    await deleteCareActivity(activity_id);
    alert('Care activity deleted successfully!');
  } catch (error) {
  ...
  const handleLogCareWithAlert = async (plant_id, activity_type) => {
  try {
    await logCareActivity(plant_id, activity_type, plants);
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
            <div>
              ...
              {profile && (
                <p className="text-sm text-gray-600">Welcome back, {profile.display_name}!</p>
              )}
            </div>
          ...
          {show_timeline && (
            <CareTimeline 
              activities={care_activities} 
              plants={plants}
              onDeleteActivity={handleDeleteCareActivity}
            />
          )}
