...
function ExpandablePlantCard({ 
  ...
  const handlePhotoClick = (e, photoIndex = 0) => {
    e.stopPropagation();
        ...
        <div className="p-4">
          ...
          {cover_photo && (
            <div className={`mb-3 transition-all duration-300 ${
              is_expanded ? 'h-40' : 'h-32'
            }`}>
              <div 
                className={`relative h-full bg-gray-100 rounded overflow-hidden cursor-pointer group ${
            ...
            <div className="space-y-2 mb-4">
              ...
              {plant.photos && plant.photos.length > 0 && (
                <p className={`text-xs ${
                  ...
                  {plant.photos.length} photo{plant.photos.length !== 1 ? 's' : ''} • Click photo to view full size
                </p>
              )}
