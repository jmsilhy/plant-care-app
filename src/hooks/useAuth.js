...
export function useAuth() {
  ...
  // Temporarily expose supabase globally for migration
useEffect(() => {
  if (supabase) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      // FIXED: Remove await from auth callback to prevent deadlock
      const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
        ...
        if (mounted) {
          ...
          // CRITICAL FIX: Wrap async operations in setTimeout as per Supabase docs
          setTimeout(async () => {
            if (session?.user) {
              await fetchUserProfile(supabase, session.user.id);
            } else {
